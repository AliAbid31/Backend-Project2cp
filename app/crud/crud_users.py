...
def forgot_password_logic(db: Session, email: str) -> bool:
    ...
    reset_request = PasswordReset(email=email, token=token, expires=expires)
    db.add(reset_request)
    db.commit()

    redirect_link = f"{BACKEND_URL}/api/users/reset-password/{token}"
    
    msg = MIMEMultipart("alternative")
    msg["Subject"] = "Reset Password - TutoratUp"
    ...
    <html>
        <body style='text-align:center; font-family: Arial, sans-serif;'>
            ...
            <a href='{redirect_link}' style='background:#007bff;color:white;padding:12px 24px;text-decoration:none;border-radius:5px;display:inline-block;'>RESET MY PASSWORD</a>
            <p style='margin-top:20px; color:#666;'>This link will expire in one hour.</p>
        </body>
    </html>
    """
...
def reset_password_logic(db: Session, token: str, new_password: str) -> bool:
