...
def get_imports():
    ...
    for root, dirs, files in os.walk(root_dir):
        if any(x in root for x in [".venv", ".git", "__pycache__", "git"]):
            continue
        for file in files:
            if file.endswith(".py"):
...
std_lib = {
    ...
    "hashlib", "hmac", "base64", "time", "asyncio", "inspect", "math", "threading", "io", "csv", 
    "shutil", "tempfile", "traceback", "enum", "uuid", "secrets", "sqlite3", "pickle", "copy"
}
